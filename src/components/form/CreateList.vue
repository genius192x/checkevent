...
<template>
  <Form v-slot="{ setFieldValue }" :validation-schema="accountFormSchema" class="space-y-6 mt-3 flex flex-col" @submit="onSubmit">
    <div class="form__fields max-h-[350px] overflow-y-auto overflow-x-visible space-y-4 md:space-y-6 md:max-h-none">
      <FormField v-slot="{ componentField }" name="name">
        <FormItem>
          ...
          <FormControl>
            <Input type="text" placeholder="Введите название" v-bind="componentField" v-model="formResult.title" />
          </FormControl>
      ...
      <FormField v-slot="{ componentField }" name="description">
        <FormItem>
          ...
          <FormControl>
            <Textarea placeholder="Введите описание" v-bind="componentField" v-model="formResult.description" />
          </FormControl>
            ...
            <PopoverTrigger as-child>
              <FormControl>
                ...
                >
                  ...
                  <span>{{ value ? df.format(toDate(dateValue, getLocalTimeZone())) : "Выберите день" }}</span>
                </Button>
              </FormControl>
            </PopoverTrigger>
        ...
        <FormItem class="flex flex-col">
          ...
          <Popover v-model:open="open">
            ...
            <PopoverContent class="w-[200px] p-0">
              <Command>
                <CommandInput placeholder="Выберите тип..." />
                <CommandEmpty>No type found.</CommandEmpty>
                <CommandList>
                  <CommandGroup>
                    <CommandItem
                        v-for="type in types" :key="type.value" :value="type.label"
                        @select="() => {
