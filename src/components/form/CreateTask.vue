...
const onSubmit = handleSubmit((values) => {
	values.id = currList.value.tasks.length + 1
	listStore.addTask(values, lastParam)
	emit('close')
	...
	<form class="space-y-8 mt-4" @submit="onSubmit">
		<FormField v-slot="{ componentField }" name="title">
			<FormItem>
				...
				<FormControl>
					<Input type="text" placeholder="Название задчи" v-bind="componentField" />
				</FormControl>
				<FormMessage />
			</FormItem>
		</FormField>
		...
		<FormField v-slot="{ componentField }" name="description">
			<FormItem>
				...
				<FormControl>
					<Textarea placeholder="Краткое (или нет) описание задачи" v-bind="componentField" />
				</FormControl>
				<FormMessage />
			</FormItem>
		</FormField>
